...
OffscreenRender::OffscreenRender(VVDevice* vv_device, VkExtent2D extent, VkCommandPool commandPool, VkQueue graphicsQueue)
    :extent(extent), commandPool(commandPool), graphicsQueue(graphicsQueue){

    device = vv_device->device();
    physicalDevice = vv_device->getPhysicalDevice(); 

    createOffscreenResources(vv_device);
    createRenderPass(vv_device);
    createFramebuffer(vv_device);
    createCommandBuffer();
...
void OffscreenRender::createOffscreenResources(VVDevice* vv_device) {
      VV_INFO("Creating offscreen image...");
    ...
    VV_INFO("Creating offscreen image view...");
    ...
    VV_INFO("Creating texture sampler...");
    ...
    depthImageInfo.format = findDepthFormat_offscreen(vv_device);
    ...
    depthImageViewInfo.format = findDepthFormat_offscreen(vv_device);
...
void OffscreenRender::createFramebuffer(VVDevice *vv_device) {
    VV_INFO("Creating framebuffer...");
    ...
    VV_INFO("Framebuffer created successfully.");
}


void OffscreenRender::createRenderPass(VVDevice* vv_device) {
    ...
    depthAttachment.format = findDepthFormat_offscreen(vv_device);
    ...
    VV_INFO("Render pass created successfully.");
}
...
void OffscreenRender::ResizeCallback(VkExtent2D newExtent, VVDevice* vv_device) {
    extent = newExtent;
...
VkFormat OffscreenRender::findDepthFormat_offscreen(VVDevice* device){
