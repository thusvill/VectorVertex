...
namespace VectorVertex
{
    ...
    {
        if (vertexBuffer == nullptr || vertexBuffer->getVKBuffer() == VK_NULL_HANDLE)
        {
            // Handle error: vertex buffer is not initialized
            std::cerr << "Error: Vertex buffer is not initialized!" << std::endl;
            return;
        }
    ...
    void VKModel::BindWithEntityID(VkCommandBuffer commandBuffer, int entityID)
    {
        if (vertexBuffer == nullptr || vertexBuffer->getVKBuffer() == VK_NULL_HANDLE)
        {
            // Handle error: vertex buffer is not initialized
    ...
    std::vector<VkVertexInputAttributeDescription> VKModel::Vertex::getAttributeDescriptions()
    {
        ...
        attributeDescriptions.push_back({4, 0, VK_FORMAT_R32_SINT, offsetof(Vertex, enttID)});
